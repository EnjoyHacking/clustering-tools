...
#sys.argv[1] : the input file of samples in numeric vector, e.g., sampleEmbedding.txt
#sys.argv[2] : the number of clusters
#sys.argv[3] : the output file of clustring results
...
class KMeans:
    ...
    def flush(self, filename):
        ...
        res = {}
        for c in xrange(self._knums):
            for e in self._clusters[c]:
                res[e[0]] = c
        for k in res.keys():
            foutput.write(str(res[k]))
            foutput.write("\n")
        print("Done.")
        foutput.close()
...
if __name__ =="__main__":
    ikmeans = KMeans(sys.argv[1], int(sys.argv[2]))
    ikmeans.process()
    ikmeans.flush(sys.argv[3])
